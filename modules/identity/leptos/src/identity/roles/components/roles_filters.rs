        ...
        <div class="grid grid-cols-1 items-start gap-3 md:grid-cols-[minmax(18rem,1fr)_auto]">
            <div class="relative flex items-center">
                ...
                <Input
                    ...
                    placeholder=i18n.t_untracked(T::RoleName).to_string()
                ...
                <div>
                    <div class="grid gap-4 rounded-md border bg-muted/30 p-4 md:grid-cols-[minmax(12rem,18rem)]">
                                <div class="grid gap-2">
                                    <Label html_for="roles-permission-filter">{move || i18n.t(T::PermissionStatus)}</Label>
                                    <SelectNative
                                        id="roles-permission-filter"
                                        value=state.permission_filter.read_only()
                                        on_change=Callback::new(move |ev| {
                                            state.permission_filter.set(event_target_value(&ev));
                                        })
                                    >
                                        <option value="all">{move || i18n.t(T::AllRoles)}</option>
                                        <option value="with-permissions">{move || i18n.t(T::RolesWithPermissions)}</option>
                                        <option value="without-permissions">{move || i18n.t(T::RolesWithoutPermissions)}</option>
                                    </SelectNative>
                                </div>
                    </div>
                </div>
