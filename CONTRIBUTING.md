...
update uses isolated `chore/github-sponsors-configuration-develop` and
`chore/github-sponsors-configuration-main` branches created from their
respective targets, the pull request title
`chore(repository): use GitHub Sponsors funding`, and no closing issue
reference. This prevents unreleased `develop` work from being carried into
`main`. Both exceptions still require review and all checks required by their
target branch.
